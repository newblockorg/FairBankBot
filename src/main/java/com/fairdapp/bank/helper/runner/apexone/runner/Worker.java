...
import com.fairdapp.bank.helper.runner.apexone.contract.Apexone;

public class Worker implements Runnable{

	private ApexOneConfig config;
	public Worker( ApexOneConfig c) {
		this.config = c;
		this.init();
	}
	private Web3j web3j ; 
	private String myselfAddress ;
	private BigInteger gasPrice =new BigInteger ("200999999998") ;
	private BigInteger gasLimit =new BigInteger ("672429") ;
	private Apexone contract;
	private Long sendTimesSecond = 1539039600L;   
	...
	public void init() {
		...
		contract 				= Apexone.load( this.config.getContractAddress() , web3j, credentials, gasPrice, gasLimit);
	...
	public void run() {
		...
		Long stageMark = 0L ;
		 ...
		 for(;;) {
				...
				try {
					 RemoteCall<Boolean> result = contract.isStarted(); ;
					 Boolean istarted  = result.sendAsync().get();
					 ...
					 if(istarted ||  ( sendTimesSecond - currentTimeSecond <1  ||  ( (sendTimesSecond - currentTimeSecond) <= 1 && minsecond < 150 ) ) ) {
